...
function write_greenfunction(am::Vector{Float64}, Greenf::Vector{ComplexF64})
    @assert length(am) == length(Greenf)

    open("Gout.data", "w") do fout
        for i in eachindex(am)
            @printf(fout, "%16.12f %16.12f %16.12f\n", am[i], real.(Greenf)[i], imag.(Greenf)[i])
        end
    end
end
...
function main()
    ...
    eta = 0.001                       # Set according to actual conditions
    ...
    write_greenfunction(omegas, G_avg)
